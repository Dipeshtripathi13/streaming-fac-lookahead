...
audible accentedness: the saturation point says marginal \emph{PER} gain falls
below the noise floor, not that listeners stop noticing improvement. Stimulus
generation is implemented and the inputs exist; only the rater study is
unfunded. This is the limitation a reader should weigh most heavily.

The audio-domain test in \S\ref{sec:rq2} closes the identity-error half of the
phoneme-class question but not the trajectory half, which a TTS cannot answer
because it renders canonical trajectories by construction; that needs a trained
synthesis pipeline aligned against a native rendition. Training is 1200 steps,
a comparison at fixed budget rather than to convergence, over a frozen encoder.
The dense sweep uses two seeds per arm with no repeated cells, so its noise
floor is imported rather than re-measured. \tbuf{}'s I/O term is unmeasured:
an acoustic loopback is defeated by platform echo cancellation, and a wired
loopback would close the one term of Eq.~\ref{eq:decomp} still open. Compute is
characterised on Apple Silicon and ARM64 Linux only; the embedded class is
