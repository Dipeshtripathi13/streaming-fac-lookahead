...
below the noise floor, not that listeners stop noticing improvement. This is
the limitation a reader should weigh most heavily.

We report why, because ``unfunded'' would be the wrong reason. Stimuli can be
built from the per-condition outputs and were, but they cannot carry the
judgement a listening test needs, for two independent reasons. The phone
sequences carry no word or stress structure, so even the ground-truth ceiling
synthesises as one unsegmented stream, \textipa{/aInEv3~2laUw2tk\ae ntbi\ldots/}
for \emph{``I never allow what can't be changed\ldots''}. And a system at
0.16--0.42 PER does not emit recognisable words to compare. An informal session
confirmed both: the listener could not identify what was being said, and scored
at chance on ground-truth anchors. \textbf{A funded rater study on these
stimuli would not have measured accentedness at any budget.} What perceptual
evaluation needs here is converted audio from a synthesis pipeline, which is
future work, not a purchase.
