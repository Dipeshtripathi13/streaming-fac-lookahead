...
\large Latency--Quality Trade-offs in Streaming Accent Conversion\\
on Commodity Hardware}

\author{Dipesh Tripathi\\
\small \todo{affiliation, email}}
...
\textbf{Nobody sweeps the budget.} PHONOS \emph{states} 40\,ms; it publishes no
ablation showing 40\,ms is necessary or sufficient. The same holds for
TVTSyn~\cite{tvtsyn2026} and DarkStream~\cite{darkstream2025}. The
question ``what would 160\,ms buy you'' has no published answer.
...
DarkStream~\cite{darkstream2025} are three systems from one group, all
GPU-evaluated, all pairing FAC or voice conversion with speaker anonymisation.
...
\todo{author list, affiliations, acknowledgements, data-availability statement,
and the official Interspeech style file. refs.bib was fully re-verified on
1 Sep 2026 (16 [V], 2 [P], 0 [U]); both remaining [P] markers are on numbers
this paper does not cite, so the bibliography is submission-clean as written}
