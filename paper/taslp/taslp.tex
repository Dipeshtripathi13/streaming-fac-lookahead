...
Second, a pre-registered hypothesis that vowels and approximants would benefit
more from future context than stops, fricatives, affricates and nasals is
\emph{not supported}: all seven manner classes improve by
...
(substitutions and deletions in the g2p--IPA alignment). Fourth, repeating the sweep on a second causalised encoder (wav2vec 2.0 base)
...
the measured exchange rate is encoder-dependent rather than a universal
property of the task. Nor does the detailed response transfer: where WavLM is smooth and
...
training seed: its response is weak and non-monotonic rather than knee-like.
Fifth, per-chunk
...
layer (\S\ref{sec:encoder}). The two start with similar PER at $L{=}0$ but
exhibit markedly different lookahead responses.}
\label{tab:encoder}
\begin{tabular}{lrrrr}
\toprule
...
both wav2vec 2.0 seeds ($+2.5\%$ and $+3.9\%$ against $+63.3\%$). \textbf{The exchange
...
there, showing that the estimator is not uniformly uninformative at six
speakers --- though that does not by itself establish power for an effect this
small.
...
substantially wider at large lookahead; the full
...
is $2.4\times$ its p50, on a signal this weak and with this much threshold
dependence, is not consistent with a stable measurement of converter latency.
We therefore reject
...
attribute the gap to pretraining scale, objective or architecture; and the layer-selection protocols are not
...
another backbone. Increasing lookahead strongly changes algorithmic latency
