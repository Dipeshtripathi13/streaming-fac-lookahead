...
canonical. Fourth, repeating the sweep on a second causalised encoder (wav2vec 2.0 base)
changes the answer quantitatively: tapped at its own best phonetic layer it
...
property of the encoder rather than of speech, while the qualitative shape
(positive, smooth, diminishing, no knee) survives the swap. Fifth, per-chunk
...
\begin{table}[t]
\centering\small
\caption{Same sweep, second encoder, each tapped at its own best phonetic
layer. The two agree at $L{=}0$ and diverge entirely thereafter.}
\label{tab:encoder}
\begin{tabular}{lrrrr}
\toprule
...
future context English phones require.} The qualitative claims survive the
swap --- returns are positive, smooth and diminishing, with no knee --- and
the quantitative ones, including the saturation point and the cost attributed
to a 40\,ms budget, should be re-measured on whatever backbone a system
actually deploys. Whether the gap traces to pretraining scale (960\,h against
...
which a further 39.8\% reduction is available. That rate, however, is measured
...
(\S\ref{sec:encoder}). The shape of the curve is the transferable finding; its
slope, its saturation point and the cost it assigns to any particular budget
must be re-measured per encoder. Lookahead changes algorithmic latency rather
