...
tapped at its own selected phonetic layer, changes the answer: it starts within
$0.01$ PER of WavLM at $L{=}0$ yet gains only $3.9\%$ relative from $640$\,ms
of lookahead against WavLM's $63.3\%$, and where WavLM is smooth and
log-linear ($R^2=0.99$) its response is weak and non-monotonic ($R^2=0.10$),
bottoming out at $160$\,ms and rising again --- a shape reproduced at a second
seed and unchanged at five times the training budget, where its endpoint gain
shrinks to $+1.8\%$ against WavLM's $+65.8\%$. Neither the measured exchange
rate nor the detailed curve shape is a universal property of the task. Fifth, per-chunk compute depends only weakly on
...
--- but they improve together, and the shape is unchanged: the minimum stays at
$L{=}160$\,ms and $L{=}640$ remains worse than it. The endpoint gain does not
grow with training; it \emph{shrinks}, from $+3.9\%$ to $+1.8\%$, while WavLM
at the same 6000 steps gains $+65.8\%$. Five times the budget widens the gap
...
recovers only $3.9\%$ of it from $640$\,ms of future context, and $1.8\%$ when
both are trained to 6000 steps (\S\ref{sec:encoder}). Neither the magnitude nor the detailed shape of the
