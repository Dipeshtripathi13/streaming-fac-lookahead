...
while separately reporting algorithmic and computational latency. We train a causal phone translator in a 16-point main sweep from 0 to 640\,ms, augmented with five targeted budgets around the diminishing-return region (21 in all),
with every other factor held fixed, and report five principal empirical
results. First,
...
(64 runs), later augmented on the conversion arm with five budgets (220, 260,
...
seed, chunk, lookback and grid fixed and changing only the checkpoint. The
matched grid is $L\in\{0,20,40,80,160,320,640\}$\,ms, a subset of the main
...
future context English phones require.} The swap rules out treating the
...
pooled result --- and not as evidence
