...
with every other factor held fixed, and report five results. First,
phone-translation PER decreases smoothly with lookahead at $-0.045$ phone error rate (PER) per
...
Instead of a knee we identify \emph{saturation onset} at ${\approx}340$\,ms:
with the grid completed to 20\,ms spacing over 200--360\,ms, and against a
fixed-seed repeatability floor measured from deliberate repeats
($2\sigma=0.0029$), the $340\rightarrow360$\,ms increment is the first to buy
less than that floor, and the average per-20\,ms gain stays below it
thereafter. Propagating the floor's bootstrap interval moves the onset over
300--480\,ms. At the 40\,ms lookahead used by PHONOS's translator, our probe
retains a further 45.3\% relative PER reduction between 40 and 340\,ms.
...
interval of $[-0.010, +0.101]$. It still fails when errors are reweighted by a synthesised spectral-distortion
proxy for acoustic cost, so this is not an artefact of PER charging every error
equally. Third, canonical-phone prediction benefits more from future context than
realized-phone transcription at matched capacity ($-0.045$ against $-0.029$
PER per doubling, $1.55\times$), and within the conversion arm the benefit is
$1.32\times$ larger in absolute PER slope at canonical-deviation positions
($\mathrm{g2p}\neq\mathrm{ipa}$). Fourth, repeating the sweep on a second causalised encoder (wav2vec 2.0 base)
changes the answer quantitatively: tapped at its own selected phonetic layer it
starts within $0.01$ PER of WavLM at $L{=}0$ yet gains only $3.9\%$ relative
from $640$\,ms of lookahead, against $63.3\%$ --- so the exchange rate is a
future-context sensitivity encoder-dependent rather than a universal property
of the task. Nor does the detailed response transfer: where WavLM is smooth and
log-linear ($R^2=0.99$), wav2vec 2.0 is weak and non-monotonic ($R^2=0.10$),
bottoming out at $160$\,ms and rising again --- a shape reproduced at a second
training seed. What survives the swap is the absence of evidence
for a sharp knee in either. Fifth, per-chunk
compute depends only weakly on
lookahead --- at most $1.9\%$ on an Apple M4 and $7.6\%$ on an ARM64 Linux
host across a $17\times$ change in algorithmic latency --- a weak empirical
dependence between lookahead and per-chunk compute over the tested range,
subject to the throughput constraint $\tcmp<c$. We release the harness and the per-condition outputs.
\end{abstract}
...
$<$1pp --- but three points scored on anonymisation accuracy cannot tightly localise
...
saturation locations with the estimate we obtain over 16 budgets.
...
(64 runs), all on an NVIDIA T4. Compute measurements use an Apple M4
(10 logical cores: 4 performance, 6 efficiency; 16\,GB; macOS 25.5) and an
...
within $0.001$, far inside the noise floor. The bracketing checkpoints agree
...
so a piecewise fit is strongly biased toward placing a breakpoint just past that gap whatever the data
does.
...
$1.1$, $1.1$. The observed $340\rightarrow360$\,ms increment is the first to
...
\subsection{The exchange rate is a property of the encoder, not of speech}
\label{sec:encoder}
...
seed, chunk, lookback and grid fixed and changing only the checkpoint. Both
encoders are causalised by the same two patches and both pass the same
truncation proof.
...
would be cleaner still to fix both layers by a pre-registered rule.

\begin{table}[t]
\centering\small
\caption{Same sweep, second encoder, each tapped at its selected phonetic
layer (\S\ref{sec:encoder}). The two agree at $L{=}0$ and diverge entirely
thereafter.}
\label{tab:encoder}
\begin{tabular}{lrrrr}
\toprule
& \multicolumn{1}{c}{$L{=}0$} & \multicolumn{1}{c}{$L{=}640$}
& \multicolumn{1}{c}{rel.\ gain} & \multicolumn{1}{c}{PER/doubling} \\
\midrule
WavLM base+ (layer 9)      & 0.4256 & 0.1561 & $+63.3\%$ & $-0.0446$ \\
wav2vec 2.0 base (layer 2) & 0.4359 & 0.4189 & $+3.9\%$  & $-0.0015$ \\
\quad \emph{seed 7}        & 0.4376 & 0.4265 & $+2.5\%$  & --- \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Direction replicates; magnitude does not.} At its own best layer
wav2vec 2.0 starts within $0.01$ PER of WavLM ($0.4359$ against $0.4256$) and
...
before rising again. The gain is nonetheless real rather than noise: $0.017$
PER absolute is $5.9\times$ the $2\sigma$ repeatability floor.
...
160, $0.4265$/$0.4189$ at 640). Both curves reach their minimum at the same
...
gain is $+2.5\%$ against $+3.9\%$: the effect is small in both, and small in
the same shape.

\textbf{This is not an artefact of tapping a shallow layer.} Layer 2 has had
almost no contextual mixing, so the natural objection is that there was little
context to withhold and that sensitivity would recover deeper in the stack. It
does not. Relative gain from $L{=}0$ to $640$\,ms is $+3.9\%$ at layer 2,
...
Two encoders that are equally good with no future context therefore differ by
a factor of $16$ in what they do with $640$\,ms of it. \textbf{The exchange
rate of $-0.045$ PER per doubling is a property of WavLM base+, and should be
read as one encoder's operating curve rather than as a fact about how much
future context English phones require.} The swap falsifies any
backbone-independent quantitative law, and it also shows that the detailed
shape need not carry over: WavLM's smooth diminishing-return curve has no
counterpart in wav2vec 2.0's weak, non-monotonic one. The only feature common
to both is the negative one --- neither shows a sharp knee. Every positive
...
We pre-registered, before the evaluated hypothesis outputs were generated, that vowels and
...
talkers \cite{hillenbrand1995} --- and that this movement is criterial rather
...
kind \cite{lehistepeterson1961}. Obstruents were placed on the other side
because stop place is signalled by brief formant transitions to a
locus \cite{delattre1955} and is recoverable from a short-time spectrum at the
release \cite{stevensblumstein1978}, so a narrow window should suffice. Testing this on 42{,}255 unique reference-phone tokens evaluated across seven
...
between $0.40$ and $0.78$ --- so every class shows a substantial positive gain;
...
$+0.0401$). No single talker drives the estimate --- the sign is stable across
all six drops --- but the entire leave-one-out range also sits inside an
...
pooled result --- the reversals show that the pooled ordering is not
consistent across the six held-out speakers --- and not as evidence
about native language.

\textbf{H2 is not supported.} The finding is not that lookahead fails to help
vowels --- every class improves 52--72\% relative. The finding is that the
pre-registered manner grouping does not reliably order lookahead benefit.
...
is also far smaller than the sequence-level test --- 1451 unique reference
...
and test split. The exchange rates separate cleanly:
\begin{center}
...
deletion over insertion, making the mapping deterministic. This is the
...
cases and never approaching zero. Recomputing the difference \emph{within each reference
...
transcription, against $[40, 180, 180]$ for conversion), so
\S\ref{sec:rq1}'s structural conclusion is not an artefact of the conversion
arm. We do not compare saturation onsets between the arms: the five budgets
added over 200--360\,ms to locate it were run on the conversion arm only, so
...
near zero at $L{=}0$ and substantially wider at large lookahead; the full
16-point curve widens in 13 of 15 adjacent steps rather than monotonically.}
\label{tab:h3dense}
\begin{tabular}{rrrr}
\toprule
...
A complementary within-run measure is the \emph{preference margin} --- PER against the other
...
by nothing. That behaviour is consistent with platform echo cancellation, which scales
...
roughly doubling at 40\,ms, which strongly suggests the reported quantity
includes queueing rather than isolated converter delay: a quantity that is
invariant to a doubling of the block size is not measuring the block.
...
is $2.4\times$ its p50 is not a latency distribution. We therefore reject
these measurements as threshold-dependent and not representative of converter
latency: what the procedure records is that a detection threshold was lowered
until the tool produced output.
...
\S\ref{sec:encoder} is two checkpoints, with the wav2vec 2.0 result replicated
at a second training seed but the WavLM curve resting on the seeds of the main
sweep; that is enough to show the exchange rate does not transfer, but not to
attribute the gap to pretraining scale, objective or architecture; and each encoder is tapped at a
single layer chosen by a probe at one lookahead, so the layer and the
lookahead sensitivity are selected on partly overlapping evidence.
...
${\approx}340$\,ms --- well beyond PHONOS's 40\,ms translator budget, relative
to which a further 45.3\% reduction is available. That rate, however, is measured
over one backbone: a second causalised encoder that matches WavLM at $L{=}0$
recovers only $3.9\%$ of it from $640$\,ms of future context
(\S\ref{sec:encoder}). Neither the magnitude nor the detailed shape of the
lookahead response transfers unchanged across the two encoders tested: WavLM
exhibits a strong smooth diminishing-return curve, whereas wav2vec 2.0 shows a
weak, non-monotonic one. Future-context budgets should therefore be
characterised for the deployed representation rather than inherited from
another backbone. Lookahead changes algorithmic latency rather
than per-chunk cost, though throughput must still satisfy $\tcmp<c$
