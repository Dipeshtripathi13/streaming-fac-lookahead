...
Instead of a knee we identify \emph{saturation} at ${\approx}240$\,ms: with the
grid completed to 20\,ms spacing over 200--360\,ms, every one of the eight
consecutive observed 20\,ms steps beyond 240\,ms buys less than the measured
$2\sigma$ repeatability floor. At the 40\,ms lookahead used by PHONOS's translator, our
...
dispersion among classes within the groups. It still fails when errors are reweighted by
...
Three results. \textbf{Training does flatten}: over the final 1500 steps every
condition moves by $0.0005$--$0.0045$ PER, at or below the $0.0044$
repeatability floor. \textbf{The ranking never reorders}: the ordering
...
$+0.0045$ (180$\rightarrow$200). Beyond 240\,ms, \textbf{every one of the eight
consecutive observed steps falls below the $0.0044$ floor}: $0.9\times$
(240$\rightarrow$260), $1.0$, $0.8$, $0.7$, $0.8$, $0.5$ out to 360\,ms, and
$0.5$ and $0.3$ for the two longer intervals to 640\,ms. We therefore place
\textbf{saturation at ${\approx}240$\,ms}, meaning the point beyond which no
observed 20\,ms increment buys a measurable gain. Unlike a knee, this depends
on the floor rather than on an axis choice.

Two caveats keep this approximate rather than sharp. The 200$\rightarrow$220
step is itself below the floor ($0.7\times$) while 220$\rightarrow$240 is above
it ($1.3\times$), and the 220\,ms condition has the largest seed spread of any
point on the curve ($0.0069$, larger than the floor itself) --- so the ordering
around 220\,ms is not resolved, and returns are already marginal from about
200\,ms. The floor is also estimated from six accidental repeats and carries no
interval of its own. Read the result as saturation setting in over roughly
200--240\,ms.
...
${\approx}240$\,ms --- well beyond PHONOS's 40\,ms translator budget, relative to
