...
\begin{abstract}
\noindent Streaming foreign accent conversion systems choose a lookahead
budget, but prior work does not densely characterise how future context trades
algorithmic latency for accuracy. We use a causal phone translator as a
controlled probe and evaluate 21 lookahead budgets from 0 to 640\,ms while
holding other factors fixed. For WavLM base+, phone error rate decreases
approximately $0.045$ per doubling of lookahead ($R^2=0.98$). Piecewise fits
indicate curvature but no identifiable knee; instead, a repeatability-aware
analysis places the onset of diminishing returns near 340\,ms, with a
300--480\,ms sensitivity range. At 40\,ms, the probe retains a further 45.3\%
relative reduction by 340\,ms. A pre-specified manner-class hypothesis is not
supported under speaker-level resampling or spectral-distortion reweighting.
Canonical-phone prediction benefits more from future context than
realized-phone transcription, with a steeper absolute slope at
canonical-deviation positions. Repeating the sweep with causalised
wav2vec 2.0 yields a weak, non-monotonic response and only 3.9\% relative
endpoint improvement, against 63.3\% for WavLM, showing that the lookahead
response is encoder dependent. Per-chunk compute varies only weakly with
lookahead; algorithmic latency and hardware throughput should therefore be
reported separately.
\end{abstract}
...
\item A latency--accuracy characterisation of a causal phone-translation probe
...
a phonetic-normalisation component directly relevant to the research question;
...
(\texttt{g2p}) and the annotated realized phone sequence (\texttt{ipa}). We operationalize the phonetic-normalisation component of accent conversion as
\begin{center}
...
phonetic-normalisation stage, and swapping the target tensor from
...
conditions that reweight any fit. Lookahead is therefore quantised in 20\,ms increments: we sample every
increment through 200\,ms and a subset of quantised budgets above it, and
...
an optimisation-rate difference rather than a context effect. We test this
